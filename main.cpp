...
#if TSAI
	printf("ErasePartition:\t\tep <PartitionName>\r\n");
#endif
	printf("-------------------------------------------------------\r\n\r\n");
}
...
#if TSAI
	printf("get_lba_from_gpt master=%p name=%s\n", master, pszName);
#endif
...
#if TSAI
bool erase_lba(STRUCT_RKDEVICE_DESC &dev, UINT uiBegin, UINT uiEnd)
{
	if (!check_device_type(dev, RKUSB_LOADER | RKUSB_MASKROM))
		return false;
	CRKUsbComm *pComm = NULL;
	...
	if (bRet) {
		while(uiBegin <= uiEnd) {
			memset(pBuf, 0, nSectorSize * DEFAULT_RW_LBA);
			uiLen = (uiEnd - uiBegin) + 1;
			uiLen =  (uiLen > DEFAULT_RW_LBA)? DEFAULT_RW_LBA: uiLen;
			iRet = pComm->RKU_WriteLBA(uiBegin, uiLen, pBuf);
			if(ERR_SUCCESS == iRet) {
				uiBegin += uiLen;
				iTotalWrite += uiLen;
				if (bFirst) {
					printf("Erase LBA (%u) + (%u) End %u\r\n", uiBegin, uiLen, uiEnd);
					bFirst = false;
				} else {
					CURSOR_MOVEUP_LINE(1);
					CURSOR_DEL_LINE;
...
#if TSAI
	else if (strcmp(strCmd.c_str(), "EP") == 0) {
		if (argc == 3) {
			bRet = read_gpt(dev, master_gpt);
			if (bRet) {
				bRet = get_lba_from_gpt(master_gpt, argv[2], &lba, &lba_end);
				if (bRet) {
					bSuccess = erase_lba(dev, (u32)lba, (u32)lba_end);
				} else
					printf("No found %s partition\r\n", argv[2]);
			}
		} else
